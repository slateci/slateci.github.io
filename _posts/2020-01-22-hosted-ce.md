...
The OSG hosts these CE's for a number of campuses, hence the name "HostedCE".  Using SLATE you can easily deploy your own  HostedCE on-premise, and scale it to as many instances as needed. It is reccomended to create a unique HostedCE instance for each individual cluster you intend to route grid jobs to.
...
This will be followed by the job's ClassAd which is a large description of the job and its state. ClassAds are also using to describe other types of objects in the CE software.

After the ClassAd thhere should be some output describing the status of the job. The job will typically go form Held to Idle. It may stay idle for a long time waiting on available resource. Eventually the trace should report that the job was successful. 
...
3) You hhave not correctly added your identity to the CE GridmapOverride

If the command hangs or you fail to read, this could indicate a connection problem or an issue with the CE.

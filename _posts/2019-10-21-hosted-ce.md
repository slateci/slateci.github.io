...
An OSG Hosted Compute Element is an application that allows a site to contribute idle HPC or HTC compute resources to the Open Science Grid. The CE is responsible for recieving jobs from the grid and routing them to your local cluster(s). These are pre-emptable jobs that will run when the cluster resources would have otherwise gone idle. 
...
## Prerequisites
You must have a functional batch system on a cluster that you like to support OSG with.
...
Create a cert and download it. 

Next you will need to convert it to PKCS12 format for voms.

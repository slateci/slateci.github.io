...
In the above example, the JupyterLab application can be accessed at <div style="display: inline">http://blogpostnotebook.slate-dev.slateci.net/</div> using the token you generated above. The second line has the SSH access info under URL, so you can use it with the ssh command like this:
